/* ═══ Spec Plan — 右侧面板：总览（审阅清单）+ 批注 + 历史 ═══
   数据全部从 REVIEW_MODEL 派生（清单数量随文档内容自动变化）。 */

const rangeLabel=(pts)=>pts.length?`${pts[0].label}–${pts[pts.length-1].label}`:'';

/* ── Tab 1 · 总览：审阅清单（防遗漏）+ 前置拍板进度 ── */
function ReviewDash({ scrollTo, signed, seenSecs }){
  const gates=REVIEW_MODEL.byKind['goal'].filter(p=>p.role==='gate');
  const decs=REVIEW_MODEL.decisions;
  const midRisks=REVIEW_MODEL.byKind['risk'].filter(p=>p.ui.lvl==='中');
  const pres=REVIEW_MODEL.byKind['precondition'];
  const opens=REVIEW_MODEL.byKind['open-question'];
  ...
  return (
    <aside className="rightbar">
      ...
      <div className="rb-body">
        {tab==='dash'&&<ReviewDash scrollTo={scrollTo} signed={signed} seenSecs={seenSecs}/>}
