/* ═══ Spec Plan — 通用 hooks ═══
   ...
   scroll-spy + 阅读进度 / 文档选区。plan-app.jsx 组合使用。 */
...
/* ── 标签跳转总线：plan-jump 事件 → 定位 + 高亮 + 记录返回点 ──
...
function useJumpBus(scrollRef){
  ...
  useEffectH(()=>{
    ...
    window.addEventListener('plan-jump',onJump);
    return ()=>window.removeEventListener('plan-jump',onJump);
  },[]);
