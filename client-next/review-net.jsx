/* ═══ Spec Plan — 后端接线层（数据 / 持久化 / 换算）═══
   ...
   契约以 types/api.ts / types/annotation.ts 为准：
     ...
     · ExtractedItem.anchor = { srcStart, srcEnd }（无 text —— 源文本由 REVIEW_DOC_SOURCE slice）
...
function applyReviewUpdate(result, content, doc){
  window.REVIEW_MODEL=buildModelFromExtract(result, doc);
  if(typeof content==='string') window.REVIEW_DOC_SOURCE=content;
}

/* 重拉 extract + 源文档 → 更新全局（agent mutated / 顶栏刷新 / 409 兜底共用）。 */
async function fetchReviewUpdate(sessionId, doc){
  const [ex, file]=await Promise.all([ getExtract(sessionId), getFile(sessionId) ]);
  applyReviewUpdate(ex.result, file.content, doc);
}
