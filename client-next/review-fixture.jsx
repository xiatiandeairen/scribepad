/* ═══════════════════════════════════════════════════════════════════
   ...
   （契约字段 vs UI 扩展字段的划分见 review-contract.jsx 头注释）。
...
/* ═══ 离线 / dev fallback 源 ═══
   live fetch（review-app bootstrap）失败时用它 buildReviewModel。此文件不再构建
   REVIEW_MODEL、也不占用全局 REVIEW_MODEL —— 全局 REVIEW_MODEL 由 review-app 从真实
   GET /api/sessions/:id/extract 经 adaptExtract 派生后写入（live 优先，单一来源，
   fixture 只在离线兜底时启用，避免 live 与 fixture 双源冲突）。 */
const REVIEW_FALLBACK_SOURCE = {
  meta: REVIEW_META,
  sections: REVIEW_SECTIONS,
  extract: { points: POINTS_FIXTURE, decisions: DECISIONS_FIXTURE },
};

Object.assign(window,{ REVIEW_FALLBACK_SOURCE });
