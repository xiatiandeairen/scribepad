/* ═══ Spec Plan — 标签芯片 / 悬停预览 / 跳转总线 ═══
   ...
   数据来源：REVIEW_MODEL.points（review-contract.jsx 派生）。 */
const { useState: useStateRef, useRef: useRefRef } = React;

const SEC_NAME = Object.fromEntries(SECTION_DEFS.map(s=>[s.id, `§${s.n} ${s.name}`]));

/* 跳转总线：任何模块 dispatch，review-app 的 useJumpBus 负责滚动 + 高亮 */
function jumpTo(label){ window.dispatchEvent(new CustomEvent('review-jump',{ detail:{ label } })); }
...
function Ref({ l }){
  ...
  const meta=REVIEW_MODEL.points[key];
