...
function buildPlanModel(source){
...
/* ═══ adaptExtract：后端结构事实 → 前端 ViewModel（DTO 第三层派生）═══
   ...
   （plan-sections.jsx）实际消费的 point.role / point.ui.* / decision.* 字段做派生，
   让任意规范书写的 8 节 plan.md 无需 per-doc overlay 即渲染。
   降级铁律：表头缺失 / 列缺失 / 无匹配时 ui 字段留空，渲染器 guard 或显示空，绝不 throw；
   每个 point 注入 ui:{} 默认——buildPlanModel 的徽标码裸访问 p.ui.lvl 等，缺则崩。 */
...
/* doc-level meta（H1 + 引导 blockquote 原文）→ PLAN_META；intro 按 '|' 拆出 status/branch。 */
function derivePlanMeta(apiMeta,docMeta){
...
/* ExtractResult → PLAN_SOURCE。uiOverlay 可选（后端格式细化时 per-doc 兜底覆盖，本期不用）。 */
function adaptExtract(apiResult,docMeta,uiOverlay){
  ...
  return {
    meta: derivePlanMeta(apiResult.meta,docMeta),
...
Object.assign(window,{ SECTION_DEFS, KIND_TO_SEC, LABEL_KINDS, KIND_META,
  normLabel, scanRefTokens, deepScanRefs, stripTokens, buildPlanModel, adaptExtract });
