 ...
 *
 * /api/rewrite is mocked deterministically (no real claude calls). The mock
...
test.describe('rewrite flow', () => {
  ...
  test('draft → instruction → AI returns → modal → accept → applied', async ({ page }) => {
    ...
    // sidebar card is gone, and the underlying document content has been
    // updated to include '[改写]'.
