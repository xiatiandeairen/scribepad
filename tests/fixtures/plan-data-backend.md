...
**选了什么**：信息点带稳定标签（`G1` / `D2` / `R3` / `P1` / `Q4`，外加目标节的已核实 bug `B1`，pattern `^[GDRPQB]\d+$`），从标题前缀 / 表格首列 / 列表加粗前缀识别；各点正文扫描 `\b[GDRPQB]\d+\b` 得 `refs`（引用即"依据"）；core 提供纯函数把引用解析成可导航关系（上下文包雏形）。有标签的点 `id = label`（重排不换 id），无标签回退 `plan-inspector` 现有结构 id 算法。
